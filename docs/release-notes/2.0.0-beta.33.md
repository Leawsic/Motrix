...
- **Settings → General → Organize downloads by file type** now follows IDM-style
  category storage, enabled by default. HTTP and direct downloads are placed under configurable
  Documents, Compressed, Music, Programs, Video, or General subfolders below
  the selected save directory. Custom names are limited to single-level folder
  names, and the task record and engine directory use the same final location.
